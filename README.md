...
- **Built-in admin UI** — modern Angular v22 SPA (signals + Signal Forms,
  spartan/ui + Tailwind v4) mounted at `/api/`, with a data editor plus
  embedded Stoplight Elements (OpenAPI) and Altair (GraphQL) explorers.
...
> The bundled Angular UI ships inside the Python wheel at
> `src/fusionserve/web/dist`. Litestar serves it via an assets
> static-files router plus a base-href-injecting index handler at
> `settings.ui_path` (default `/api/-/`); if the directory is missing
> or `index.html` is absent, requests to `<ui_path>` 404 at request
> time. For frontend development run `pnpm run dev` in `ui/` — the
> Angular dev server starts at `http://localhost:5173/` and proxies
> `/api/*` to the backend on `:8001`.
...
| `ui_path` | `f"{base_path}/-/"` (i.e. `/api/-/`) | Public URL where the Angular SPA is mounted. `/api/` issues a 302 redirect here. Derives from `base_path` when unset; override with `UI_PATH=…`. |
...
The bundled Angular SPA is served by two handlers wired up in
`fusionserve.ui` at `settings.ui_path` (default `/api/-/`, derived
from `base_path`): an assets static-files router at `<ui_path>assets`
and a base-href-injecting `index.html` handler that also serves the
deep-link fallback. Users typically arrive via a 302 redirect from
`/api/` to `settings.ui_path` issued by
`fusionserve.ui.RedirectRenderPlugin`. Angular emits its whole browser
bundle (with *relative* asset URLs) under `assets/`, so the static
router serves the hashed chunks at `<ui_path>assets/...`; the index
handler rewrites `<base href>` to `<ui_path>assets/` at serve time and
the Angular router derives its base one level up (`<ui_path>`) via
`APP_BASE_HREF` — so the SPA is location-independent and can be
relocated by changing only `ui_path`. The SPA bundles Stoplight
Elements for the OpenAPI page and Altair for the GraphQL page; the raw
OpenAPI 3.1 document at `/api/openapi.json`, the Scalar viewer at
`/api/scalar`, Swagger UI at `/api/swagger` and the GraphiQL IDE at
`/api/graphql` remain available for direct access. Client-side
navigation uses browser-history (path) routing (`/api/-/data`,
`/api/-/openapi`, …).

For frontend development run `pnpm run dev` in `ui/`: the Angular dev
server starts at `http://localhost:5173/` and proxies `/api/*`
requests (REST, GraphQL, OpenAPI surfaces) to the Litestar backend on
`:8001`. In dev the SPA is reached at the dev-server root (Angular's
history fallback serves `index.html`), not at `<ui_path>`; the router
base is read from `document.baseURI` so deep links reload cleanly in
both dev and prod.
...
schema). The Angular SPA at `settings.ui_path` renders the document
with Stoplight Elements; the top-level `/api/` URL redirects there.
...
the same path when accessed via a browser; the Angular SPA's GraphQL
page embeds the Altair IDE against the same endpoint.
...
├── ui.py          # Angular SPA wiring + RedirectRenderPlugin
├── auth.py        # JWT verification and User model
└── models.py      # Pydantic / Strawberry helper models

ui/                # pnpm + Angular v22 + spartan/ui + Tailwind v4 SPA
...
| Path | Description |
|---|---|
| `/api/` | 302 redirect to the Angular UI at `settings.ui_path` (default `/api/-/`). |
...
| `<ui_path>` (default `/api/-/`, and any `<ui_path>/{path:path}`) | Angular SPA index + deep-link fallback; hashed browser bundle under `<ui_path>assets/`. Configurable via `settings.ui_path`. |
