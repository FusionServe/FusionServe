...
| 13 | **WS4 pagination via relay connections:** top-level single-PK list queries are now relay connections (`edges`/`node`/`pageInfo`/`totalCount`, `first`/`after`). Two consequences: (a) node types expose `id` as an opaque relay `GlobalID`, but the by-pk lookup and CRUD mutations still take/return the **raw** integer PK — a GlobalID↔raw-int mismatch clients must reconcile (or decode); (b) composite-PK tables can't carry a single `relay.NodeID` and fall back to a plain `list` field. | Med | Accepted for the spike (tests decode the GlobalID for chaining). A full migration should either expose the raw PK as a field, or move pk-lookup/mutations to GlobalID-based `node(id:)` semantics for consistency. Composite-PK connection support would need a custom cursor/NodeID scheme. |
