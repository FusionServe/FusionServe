...
| 4 | No description hook on dynamic `type()` → column/table SmartComment descriptions are lost on output types. | Med | **Resolved (WS3):** post-process `gql_type.__strawberry_definition__` after decoration to set the type description (table comment) and per-field descriptions (column comments). Proven by `test_graphql_column_description`. |
...
| 8 | Spike reaches into private backend attrs (`_filter_registry`, `_order_registry`). | Med | **Contained (WS6):** access is funnelled through `_orm_registry()` which raises a clear upgrade error if the attribute disappears, and `tests/test_graphql_orm_contract.py` pins the private API (+ the context stash slots) so a breaking strawberry-orm upgrade fails loudly. Still fragile by nature; no public accessor in 0.13.0. |
...
| 12 | Selecting a **relation inside a mutation payload** (e.g. `updateAuthor(...) { books { ... } }`) triggers an async lazy-load and fails with `greenlet_spawn has not been called`, because mutation resolvers return raw ORM instances the optimizer never eager-loaded. | Med | Spike limitation: mutation payloads should select scalar columns only. A full migration would eager-load (or re-issue an optimized SELECT for) the returned rows so payload relations resolve. Not yet implemented. |
| 13 | strawberry-orm's relay `ORMListConnection` requires `relay.Node`: it hides native `id`/PK columns behind an opaque `GlobalID` and offers no composite-PK support. | Med | **Resolved (WS8):** replaced by a dedicated `fusionserve.connections` module — a custom `Connection[T]` (no `relay.Node`) supporting **composite PKs**, native PK columns, cursor (keyset) + limit/offset pagination, both `edges{cursor node}` and flat `nodes` shapes, and `pageInfo`/`totalCount`. Cursor = `base64("<Type>:<v1|v2|…>")` over the effective sort key (order columns + PK tiebreakers). pk-lookup and mutations return raw PKs again. Requires extending the optimizer's `_RELAY_PASSTHROUGH_FIELDS` with `nodes` (pinned by a contract test). |
