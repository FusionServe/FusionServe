"""Application dependency generators.

This module contains functions to create dependency providers for services and filters.

Adapted from the advanced-alchemy Litestar plugin, with modifications to fit the naming convention.
"""
...
class FilterConfig(TypedDict):
    ...
    """Indicates that the id filter should be enabled. When set,
    the type specified will be used for the :class:`CollectionFilter`."""
...
class DependencyCache(metaclass=SingletonMeta):
    """Simple dependency cache for the application.
    This is used to help memoize dependencies that are generated dynamically."""
...
@overload
def create_service_provider(  # noqa: UP047
...
) -> Callable[..., AsyncGenerator[AsyncServiceT_co]]: ...
...
def create_service_provider(  # noqa: UP047
    ...
    uniquify: bool | None = None,
    count_with_window_function: bool | None = None,
) -> Callable[..., Generator[SyncServiceT_co]]: ...
...
def create_service_provider(  # noqa: UP047
    ...
    uniquify: bool | None = None,
    count_with_window_function: bool | None = None,
) -> Callable[..., AsyncGenerator[AsyncServiceT_co] | Generator[SyncServiceT_co]]:
    ...
    if issubclass(service_class, SQLAlchemyAsyncRepositoryService) or service_class is SQLAlchemyAsyncRepositoryService:  # type: ignore[comparison-overlap]
        ...
        return_type_annotation = AsyncGenerator[service_class]  # type: ignore[valid-type]

        async def provide_service_async(*args: Any, **kwargs: Any) -> AsyncGenerator[AsyncServiceT_co]:
            db_session = cast("AsyncSession | None", args[0] if args else kwargs.get(session_dependency_key))
            async with service_class.new(  # type: ignore[union-attr]
                session=db_session,  # type: ignore[arg-type]
                statement=statement,
                config=cast("SQLAlchemyAsyncConfig | None", config),  # type: ignore[arg-type]
    ...
    return_type_annotation = Generator[service_class]

    def provide_service_sync(*args: Any, **kwargs: Any) -> Generator[SyncServiceT_co]:
        db_session = cast("Session | None", args[0] if args else kwargs.get(session_dependency_key))
        with service_class.new(
            session=db_session,
            statement=statement,
            config=cast("SQLAlchemySyncConfig | None", config),
            error_messages=error_messages,
            load=load,
            execution_options=execution_options,
            uniquify=uniquify,
...
def create_service_dependencies(  # noqa: UP047
    service_class: type[AsyncServiceT_co | SyncServiceT_co],
    /,
    key: str,
    statement: Select[tuple[ModelT]] | None = None,
    ...
    if issubclass(service_class, SQLAlchemyAsyncRepositoryService) or service_class is SQLAlchemyAsyncRepositoryService:  # type: ignore[comparison-overlap]
        svc = create_service_provider(  # type: ignore[type-var,misc,unused-ignore]
            service_class,
            statement,
            cast("SQLAlchemyAsyncConfig | None", config),
            error_messages,
            load,
            execution_options,
            uniquify,
        ...
        svc = create_service_provider(  # type: ignore[assignment]
            service_class,
            statement,
            cast("SQLAlchemySyncConfig | None", config),
            error_messages,
            load,
            execution_options,
            uniquify,
...
def _create_statement_filters(  # noqa: C901
    ...
    if config.get("id_filter", False):
        ...
        def provide_id_filter(  # pyright: ignore[reportUnknownParameterType]
            ids: list[str] | None = Parameter(query="ids", default=None, required=False),  # noqa: B008
    ...
    if config.get("pagination_type") == "limit_offset":
        ...
        def provide_limit_offset_pagination(
            ...
            limit: int = Parameter(
                query="_limit",
                ge=1,
                default=config.get("pagination_size", dep_defaults.LIMIT),
                required=False,
            ),
        ) -> LimitOffset:
    ...
    if search_fields := config.get("search"):
        ...
        def provide_search_filter(
            ...
            ignore_case: BooleanOrNone = Parameter(  # noqa: B008
                ...
                default=config.get("search_ignore_case", False),  # noqa: B008
                required=False,
            ),
        ) -> SearchFilter:
    ...
    if sort_field := config.get("sort_field"):
        ...
        def provide_order_by(
            ...
            sort_order: SortOrderOrNone = Parameter(  # noqa: B008
                title="Field to search",
                query="sortOrder",
                default=config.get("sort_order", "desc"),  # noqa: B008
                required=False,
            ),
        ) -> OrderBy:
        ...
        for field_def in not_in_fields:
            ...
            def create_not_in_filter_provider(  # pyright: ignore
                ...
                def provide_not_in_filter(  # pyright: ignore
                    values: list[field_name.type_hint] | None = Parameter(  # noqa: B008 # type: ignore
                        query=camelize(f"{field_name.name}_not_in"),  # noqa: B008
                        default=None,
                        required=False,  # noqa: B008
                    ),
    ...
    if in_fields := config.get("in_fields"):
        ...
        for field_def in in_fields:
            ...
            def create_in_filter_provider(  # pyright: ignore
                field_name: FieldNameType = field_def,  # type: ignore[assignment]
            ) -> Callable[..., CollectionFilter[Any] | None]:
                def provide_in_filter(  # pyright: ignore
                    values: list[field_name.type_hint] | None = Parameter(  # type: ignore # noqa: B008
                        query=camelize(f"{field_name.name}_in"),  # noqa: B008
                        default=None,
                        required=False,  # noqa: B008
                    ),
                ) -> CollectionFilter[field_name.type_hint] | None:  # type: ignore # pyright: ignore
                    return (
...
def _create_filter_aggregate_function(config: FilterConfig) -> Callable[..., list[FilterTypes]]:  # noqa: C901, PLR0915
    ...
    def provide_filters(**kwargs: FilterTypes) -> list[FilterTypes]:
        ...
        if (
            (order_by := cast("OrderBy | None", kwargs.get("order_by_filter")))
